...
An `src/vacations.py` anhängen (die Importzeile oben um `secrets`, `threading`, `json_store`, `time_utils` ergänzen):
...
class VacationStore:
    ...
    def get_all(self) -> dict[str, Vacation]:
        ...
        with self._lock:
            return {pid: dict(p) for pid, p in self._data.items()
    ...
    def save(self, period_id: str | None, name: str, date_from: str,
        ...
        with self._lock:
            ...
            if collision is not None:
                raise ValueError(
                    f"Zeitraum überschneidet sich mit dem Urlaub „{collision}\".")
            pid = period_id or secrets.token_hex(4)
    ...
    def delete(self, period_id: str) -> None:
        """Tombstone schreiben (days={}). Der Tombstone bleibt, bis der
        gcal-Reconcile das zugehörige Event entfernt hat — analog
        reservations.py."""
        with self._lock:
...
```python
    # Urlaubsperioden. Am geteilten data_lock wie die übrigen Stores; reist
    # NICHT per Drive-Sync (gerätelokal, wie webhook_store) und ist bewusst
    # NICHT an gcal_enabled gekoppelt — Urlaub funktioniert ohne Google.
...
Run: `pytest tests/test_grid_renderer.py -k vacation -v`
Expected: FAIL mit `TypeError: _build_tooltip_text() got an unexpected keyword argument 'vacation'`
...
In `src/grid_renderer.py`: den Import um `VACATION_BG, VACATION_BG_HOVER, VACATION_ACCENT` erweitern, `vacation_store` in `__init__` aufnehmen (nach `reservation_store`, Default `None`, als `self._vacation_store` ablegen), und die neue Zelle ergänzen:

```python
    def _build_vacation_cell(self, parent, date_str, day_text, minutes,
...
```python
    def _build_day_cell(self, parent, date_str, day_text, day_date, is_weekend,
                        ...
                        reservation=None, vacation=None, vacation_minutes=0):
        ...
        if entry:
            ...
            cell = self._build_vacation_cell(
                parent, date_str, day_text, vacation_minutes,
                cell_size=cell_size, time_font=entry_time_font,
            )
        elif is_holiday:
...
In `_refresh_month` und `_refresh_week` **einmal pro Refresh** die beiden Sichten holen (nicht pro Zelle) und je Tag durchreichen. Vor der Zellschleife:

```python
        vacation_minutes_map = (
            self._vacation_store.day_minutes() if self._vacation_store else {})
        vacation_periods = (
            self._vacation_store.get_all() if self._vacation_store else {})
```

Und für die Zuordnung Tag → Periode ein kleiner Helfer als Methode (Perioden überschneiden sich nicht, es kann also höchstens eine sein):

```python
    @staticmethod
    def _vacation_for(vacation_periods, date_str):
        """Die Periode, die diesen Tag abdeckt, um ihre `id` ergänzt — oder
        None. Arbeitet auf dem einmal pro Refresh geholten Snapshot, statt je
        Zelle den Store zu fragen."""
        for pid, period in vacation_periods.items():
            if date_str in period.get("days", {}):
                return {**period, "id": pid}
        return None
```
...
```python
                    vacation=self._vacation_for(vacation_periods, date_str),
                    vacation_minutes=vacation_minutes_map.get(date_str, 0),
```

In `src/ui.py` beim `GridRenderer(...)`-Aufruf (`ui.py:120`) `self.vacation_store` nach `self.reservation_store` mitgeben.
...
- [ ] **Step 11: Manuell prüfen (Tk-gebunden, nicht automatisiert testbar)**
...
Expected: Im September 2026 sind alle 14 Tage türkis; Wochenenden zeigen „Urlaub" ohne Stundenzeile, Werktage „Urlaub" + „8 h". Hover zeigt Namen und Zeitraum. Danach die Datei wieder entfernen (`rm vacations.json`), damit kein Testdatensatz im Repo-Verzeichnis liegen bleibt.
...
def test_delete_button_defaults_to_no_vacation():
    # Bestehende Aufrufer geben das dritte Merkmal nicht mit.
...
und den Refresh-Block erweitern:
...
def plan_vacation_save(name, date_from, date_to, mode, value, overrides, state):
    ...
    if mode == "total":
        ...
        for day, minutes in zip(sorted(workdays), parts):
            days[day] = minutes
    else:
...
def _open_edit_dialog(parent, vacation_store, settings, period_id, on_saved):
    ...
    value_var = tk.StringVar(
        value=str(settings.get("vacation_hours_per_day") or 8.0))
    ...
    for mode, label in (("per_day", "Stunden pro Tag"),
        ...
        tk.Radiobutton(
            ...
            command=lambda: _recalc(), font=FONT, bg=BG, fg=TEXT,
    ...
    value_var.trace_add("write", lambda *_: _recalc())

    overrides = dict(existing["days"]) if existing else {}
    day_rows_frame = tk.Frame(dialog, bg=BG)
    day_vars = {}
    expanded = {"on": False}
    ...
    day_rows_frame.pack(anchor="w", padx=24)
    day_rows_frame.pack_forget()
    ...
    def _current_plan():
        ...
        return plan_vacation_save(
            name_var.get(), df.isoformat(), dt.isoformat(),
    ...
    def _rebuild_day_rows(days):
        ...
        for day in sorted(days):
            ...
            var = tk.StringVar(value=f"{days[day] / 60:.2f}".replace(".", ","))
            day_vars[day] = var
    ...
    def _override(day):
        try:
            ...
            return
        overrides[day] = max(0, int(round(hours * 60)))
        _update_total()
    ...
    def _recalc():
        # Eine Änderung an Zeitraum oder Sammelfeld verwirft die
        # Einzeltag-Übersteuerungen: sie bezogen sich auf den alten Stand.
        overrides.clear()
        plan = _current_plan()
        if expanded["on"]:
    ...
    def _toggle_days():
        ...
        if expanded["on"]:
            ...
            day_rows_frame.pack(anchor="w", padx=24)
            toggle.config(text="▾ Einzelne Tage anpassen")
        else:
            day_rows_frame.pack_forget()
            toggle.config(text="▸ Einzelne Tage anpassen")
...
```python
    def _on_vacation_change(self):
        """Nach einer Urlaubsänderung: Kalender neu zeichnen und — bei
        aktivem Kalender-Sync — den Push anstoßen. Urlaub selbst ist NICHT an
        gcal_enabled gekoppelt; nur der Push ist es."""
        self._refresh()
        self._bg.trigger_reconcile(self._on_reconcile_done)
```
...
3. Erneut „Neu" mit überlappendem Zeitraum → themed Fehlermeldung, die den Namen des kollidierenden Urlaubs nennt.
...
In `tests/test_report.py` anhängen (die Datei zuerst lesen; sie nutzt `ist_slot as _slot` und `make_fake_xhtml2pdf` aus `tests/conftest.py`):

```python
def test_report_without_vacation_days_is_unchanged():
    entries = {"2026-07-01": {"slots": [_slot("08:00", "16:00", pause=30)]}}
    ohne, _ = report.generate_report(
        datetime.date(2026, 7, 1), datetime.date(2026, 7, 31), entries)
    mit_leer, _ = report.generate_report(
        datetime.date(2026, 7, 1), datetime.date(2026, 7, 31), entries,
        vacation_days={})
...
def _build_vacation_summary(vacation_days: dict[str, int],
    ...
    ({ISO: minutes}). Tage mit 0 Minuten (Wochenende, Feiertag) zählen nicht —
    liegen NUR solche im Zeitraum, entsteht kein Block.
    ...
    aus derselben Auflösung, damit die Abrechnungssumme nicht an zwei Stellen
    unabhängig gerundet wird.
    """
    paid_days = {d: m for d, m in vacation_days.items() if m}
    if not paid_days:
        return ""

    s = style
    td = s["td_base"]
    vacation_minutes = sum(paid_days.values())
    label = (f"{format_date(datetime.date.fromisoformat(min(paid_days)))} – "
             f"{format_date(datetime.date.fromisoformat(max(paid_days)))}")
    ...
    return (
        ...
        f"<td style='{td}{s['c_hours']}'>{_minutes_as_hours(vacation_minutes)}h</td>"
        f"</tr>"
...
def _minutes_as_hours(minutes: int) -> float:
    ...
    nachdem in Minuten summiert wurde."""
    return round(minutes / 60, 2)


def _work_minutes(range_entries: dict[str, Any]) -> int:
    """Ist-Zeit-Minuten des gefilterten Zeitraums.

    Über `hours_to_minutes` je Tag — dieselbe Auflösung, in der der Kalender
    seine Zellen anzeigt und der Footer summiert (CLAUDE.md: Summen über
    Minuten, nie über Dezimalstunden)."""
    return sum(hours_to_minutes(_entry_hours(e)) for e in range_entries.values())
```

Die Importzeile aus `time_utils` in `report.py` um `hours_to_minutes` erweitern.
...
Nach dem `category_summary`-Block:

```python
    vacation_in_range = filter_period(date_from, date_to, vacation_days or {}) or {}
    vacation_summary = _build_vacation_summary(
        vacation_in_range, _work_minutes(range_entries), HTML_STYLE)
```

und im HTML-Template `{vacation_summary}` direkt nach `{category_summary}` einfügen.

In `generate_pdf` dieselben drei Änderungen mit `PDF_STYLE` und `{vacation_summary}` im PDF-Template.
...
git commit -m "feat(urlaub): Urlaubs-Block und Zeile 'Zu verguetenn gesamt' in Bericht und PDF"
```

---

### Task 8: Häkchen und Durchreichung
...
**Interfaces:**
- Consumes: `generate_report`/`generate_pdf` mit `vacation_days` aus Task 7
- Produces: `_PeriodPickerHandle.get_show_vacation() -> bool`; `build_period_picker(..., vacation_store=None)`

- [ ] **Step 1: Write the failing tests**

In `tests/test_period_picker.py` anhängen (die Datei zuerst lesen — sie testet die Tk-freien Helfer wie `selected_category_filter`):

```python
def test_vacation_snapshot_is_filtered_to_period():
    """Der Picker reicht den ungefilterten Snapshot durch; geschnitten wird
    erst im Report — genau wie bei den Entries. Hier nur der Vertrag, dass
    der Snapshot flach und ISO-keyed ankommt."""
    from src.report import filter_period
    import datetime

    snapshot = {"2026-07-01": 480, "2026-08-01": 480}
    assert filter_period(datetime.date(2026, 7, 1), datetime.date(2026, 7, 31),
                         snapshot) == {"2026-07-01": 480}
```
...
```python
# In _kwargs die base-dict-Zeile erweitern:
#     categories=None, category_breakdown=False, vacation_days=None,
...
In `src/dialogs/period_picker.py`: `_PeriodPickerHandle.__init__` um `vacation_var` erweitern und die Lesemethode ergänzen:
...
Dieselbe Änderung in `src/dialogs/send_dialog.py` an allen drei Stellen (`232`, `275`, `297`) — dort geht `vacation_days` an `send_task.perform_send`.
...
Dasselbe in `src/dialogs/send_task.py::perform_send` (`send_task.py:62`): Parameter `vacation_days=None` in die Signatur und an den `generate_pdf`-Aufruf (`send_task.py:89`) sowie an `build_json_payload` (Task 9) weiterreichen. Der `generate_report`-Aufruf im Sende-Dialog (`send_dialog.py:275`) bekommt ihn ebenfalls.
...
**Interfaces:**
- Consumes: `filter_period` (vorhanden), `vacation_days`-Snapshot aus Task 8
...
In `tests/test_webhook.py` anhängen:
...
- Modify: `src/background_tasks.py` bzw. `src/sync_runtime.py` (Aufruf im bestehenden Reconcile-Pfad)
...
def parse_vacation_event(event):
    """Calendar-Event → {period_id, event_id, modified_at} oder None, wenn es
    nicht den Urlaubs-Marker trägt."""
...
Urlaubstermin würde also ohnehin nie erkannt. Der einzige Verlust ist: wer das
Event in Google verschiebt, bekommt es beim nächsten Abgleich zurückgesetzt.
Ein Import ist als eigener, späterer Schritt vorgesehen.
...
def plan_vacation_sync(local_raw: dict[str, Vacation],
    ...
    - Lebende Periode, deren modified_at neuer ist als das des Events → update.
    - Tombstone mit Event → delete (danach darf der Tombstone weg).
    - Remote-Event ohne lebende lokale Periode → delete (Verwaiste).
    """
    by_period = {ev["period_id"]: ev for ev in remote_events}
    create: list[Any] = []
    update: list[Any] = []
    delete: list[Any] = []

    for pid, period in local_raw.items():
        remote = by_period.pop(pid, None)
        if period.get("deleted"):
    ...
    delete.extend(ev["event_id"] for ev in by_period.values())
...
Und im `try`-Block **nach** dem Reservierungs-Reconcile, vor
`sync_history.mark_reconciled(base)`:

```python
        # Urlaubs-Push in denselben Lauf, hinter den Reservierungen: derselbe
        # Service, dasselbe gcal_enabled-Gate, derselbe data_lock. Die beiden
        # Abgleiche stören sich nicht — sie filtern serverseitig auf
        # verschiedene Marker-Werte (reservation vs. vacation).
        if vacation_store is not None:
...
dort, wo sie heute `reservation_store` weitergeben. `BackgroundTaskRunner`
bekommt den Store dafür in `__init__` (in `ui.py:112` mitgeben, neben
...
```bash
git add src/gcal.py src/vacations_sync.py src/sync_runtime.py src/background_tasks.py tests/test_vacations_sync.py tests/test_gcal.py tests/test_type_annotations.py
...
Das ist Absicht — Urlaubsstunden dürfen die 20-h-Warnung nicht auslösen.

Der Google-Kalender-Push ist ein **Zusatz**, keine Voraussetzung: anders als
Reservierungen ist Urlaub **nicht** an `gcal_enabled` gekoppelt.
```
...
Im Abschnitt zur Google-Anbindung ergänzen:
...
Nach Task 11 sind alle Akzeptanzkriterien aus #88 erfüllt:

| Kriterium | Erfüllt in |
|---|---|
| Urlaub A–B anlegen, im Kalender ohne Hover erkennbar, per Rechtsklick löschbar | Task 4, 5, 6 |
| Funktioniert ohne jede Google-Anbindung | Task 1–8 (kein gcal-Gate); Task 10 ist additiv |
| Stunden-Logik Tk-frei und getestet | Task 1 (`tests/test_vacation_rules.py`) |
...
| Versand: Häkchen wirkt in Mail-HTML **und** PDF identisch | Task 7 (gemeinsamer Baustein `_build_vacation_summary`) |
| Beide `CLAUDE.md` mitgepflegt | Task 11 |

Bewusst **nicht** umgesetzt (im Issue als eigene, spätere Schritte zu vermerken): Drive-Sync (Schema 4→5), Share-Doc (v3→v4), Urlaub im Kalender-Footer, Import aus dem Google Kalender, Urlaubskonto/Resttage.

Empfohlene Reihenfolge für einen Zwischen-Merge: Task 1–6 ergeben ein vollständig benutzbares Feature ohne Bericht und ohne Google — ein sinnvoller PR-Schnitt, falls der Umfang in zwei Teile soll.
