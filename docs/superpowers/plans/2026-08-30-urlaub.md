...
@pytest.mark.parametrize("reader", ["get", "get_all", "get_all_raw",
                                    "period_for_date"])
def test_returned_records_do_not_share_the_days_dict(store, reader):
    """Der Dialog bearbeitet `days` des zurückgegebenen Records — das darf bei
    KEINEM der vier Leser am Store vorbei in dessen Speicher schreiben.
    Parametrisiert, damit ein spaeter weggelassenes _copy auffliegt."""
    pid = store.save(None, "Sommer", "2026-07-01", "2026-07-01",
                     _days(("2026-07-01", 480)))
    if reader == "get":
        period = store.get(pid)
    elif reader == "get_all":
        period = store.get_all()[pid]
    elif reader == "get_all_raw":
        period = store.get_all_raw()[pid]
    else:
        period = store.period_for_date("2026-07-01")
        ...
        with self._lock:
            for pid, period in self._data.items():
                ...
                if date_str in period.get("days", {}):
                    # ueber _copy, wie die drei anderen Leser: ein flacher
                    # Spread teilte das verschachtelte days-Dict mit dem Store,
                    # und eine Mutation am Rueckgabewert schriebe am
                    # _save_to_disk vorbei in dessen Speicher.
